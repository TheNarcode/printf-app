...
const PrintPreview = memo(({file, colorMode, paperSize, sides}: PrintPreviewProps) => {
  ...
  return (
    <View style={[styles.container, {backgroundColor: colors.card, borderColor: colors.border}]}>
      {isImage && file.uri ? (
        ...
        <View style={[styles.docPreview, {backgroundColor: colors.surface}]}>
          <View style={[styles.docIconBox, {backgroundColor: fileColor + '15'}]}>
            {isImage ? (
          ...
          <Text style={[styles.fileName, {color: colors.text}]} numberOfLines={2}>{file.name}</Text>
      ...
      <View style={styles.settingsRow}>
        <View style={[styles.tag, {backgroundColor: colors.surface}]}>
          <Text style={[styles.tagText, {color: colors.textSecondary}]}>{paperSize.toUpperCase()}</Text>
        </View>
        <View style={[styles.tag, {backgroundColor: colors.surface}]}>
          <Text style={[styles.tagText, {color: colors.textSecondary}]}>{colorMode === 'color' ? 'Color' : 'B&W'}</Text>
        </View>
        <View style={[styles.tag, {backgroundColor: colors.surface}]}>
          <Text style={[styles.tagText, {color: colors.textSecondary}]}>{sides === 'double' ? 'Double-sided' : 'Single-sided'}</Text>
        </View>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  fileName: {fontSize: fontSize.md, fontWeight: fontWeight.semibold, textAlign: 'center'},
  ...
  tagText: {fontSize: fontSize.xxs, fontWeight: fontWeight.medium, textTransform: 'uppercase', letterSpacing: 0.3},
});
