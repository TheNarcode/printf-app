  ...
  return (
    <View style={[styles.container, {backgroundColor: colors.card, borderColor: colors.border}]}>
        ...
        {periods.map(p => {
          ...
          return (
            <TouchableOpacity
              ...
              <Text weight={active ? 'semibold' : 'medium'} style={[
                styles.periodText,
      ...
      <View style={styles.totalSection}>
        <Text weight="bold" style={[styles.totalLabel, {color: colors.textMuted}]}>TOTAL SPENT</Text>
        <Text weight="bold" style={[styles.amount, {color: colors.text}]}>{formatCurrency(summary.totalSpent)}</Text>
      </View>
      ...
      <View style={styles.grid}>
        <View style={[styles.gridItem, {backgroundColor: colors.surface, borderColor: colors.border}]}>
          <View style={styles.gridHeader}>
            ...
            <Text weight="bold" style={[styles.gridLabel, {color: colors.textMuted}]}>ORDERS</Text>
          </View>
          <Text weight="bold" style={[styles.gridValue, {color: colors.text}]}>{summary.orderCount}</Text>
        </View>

        <View style={[styles.gridItem, {backgroundColor: colors.surface, borderColor: colors.border}]}>
          <View style={styles.gridHeader}>
            <Layers size={moderateScale(12)} color={colors.textMuted} strokeWidth={2} />
            <Text weight="bold" style={[styles.gridLabel, {color: colors.textMuted}]}>PAGES</Text>
          </View>
          <Text weight="bold" style={[styles.gridValue, {color: colors.text}]}>{summary.pageCount}</Text>
        </View>

        <View style={[styles.gridItem, {backgroundColor: colors.surface, borderColor: colors.border}]}>
          <View style={styles.gridHeader}>
            <Circle size={moderateScale(12)} color={colors.textMuted} strokeWidth={2} />
            <Text weight="bold" style={[styles.gridLabel, {color: colors.textMuted}]}>B&W</Text>
          </View>
          <Text weight="bold" style={[styles.gridValue, {color: colors.text}]}>{summary.bwPages}</Text>
        </View>

        <View style={[styles.gridItem, {backgroundColor: colors.surface, borderColor: colors.border}]}>
          <View style={styles.gridHeader}>
            <Palette size={moderateScale(12)} color={colors.textMuted} strokeWidth={2} />
            <Text weight="bold" style={[styles.gridLabel, {color: colors.textMuted}]}>COLOR</Text>
          </View>
          <Text weight="bold" style={[styles.gridValue, {color: colors.text}]}>{summary.colorPages}</Text>
        </View>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  periodText: {fontSize: moderateScale(11)},
  
  totalSection: {alignItems: 'center', marginBottom: scale(24)},
  totalLabel: {fontSize: moderateScale(9), letterSpacing: 1.2, textTransform: 'uppercase', marginBottom: scale(4)},
  amount: {fontSize: moderateScale(34), letterSpacing: -0.5},
  ...
  gridLabel: {fontSize: moderateScale(9), letterSpacing: 1, textTransform: 'uppercase'},
  gridValue: {fontSize: moderateScale(20)},
});
