...
export async function getSafePreviewUri(uri: string): Promise<string> {
  try {
    ...
    if (realUri.startsWith('content://')) {
      const dest = `${ReactNativeBlobUtil.fs.dirs.CacheDir}/safe_${Date.now()}.pdf`;
...
export async function generatePreviewPdf(uri: string, settings: PrintSettings): Promise<string | null> {
  try {
    const realUri = uri.replace('file://', '');
    ...
    try {
      ...
      console.warn('Could not read local file (likely a mock URI). Generating blank PDF for preview.');
      // Create a dummy 5-page PDF for preview purposes
      const dummyDoc = await PDFDocument.create();
      for (let i=0; i<5; i++) {
        const page = dummyDoc.addPage();
        page.drawText(`Mock Document Preview\nPage ${i+1}\n\nActual file not found on device.`, {x: 50, y: 700, size: 24});
      }
    ...
    const tempPath = `${ReactNativeBlobUtil.fs.dirs.CacheDir}/preview_${Date.now()}.pdf`;
